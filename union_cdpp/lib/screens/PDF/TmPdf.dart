...
class TmPdf {
  ...
  static Future<Uint8List> loadImageV1() async {
    ...
    String mapUrl =
        'https://flexwill.duckdns.org:7080/https://naveropenapi.apigw.ntruss.com/map-static/v2/raster?w=400&h=420&center=$longitude,$latitude&level=17&scale=2&scaleControl=true';
  ...
  static Future<Uint8List> generateTM(
      PdfPageFormat format, ssoMember? ssoUser1) async {
    ...
    dealNo = GV.pStrg.getXXX(Param_myDealNo);
    pdfDealType = GV.pStrg.getXXX(Param_dealType);
    ...
    ssoMember? ssoUser = ssoUser1;
    MyInfoItem? data = IdApi.LoggedUser();
    data!.s3FileUrl = ssoUser!.s3FileUrl;
    data!.ncardUrl = ssoUser!.s3FileUrl;

    phone = data.userTel!.substring(0, 3) +
        '-' +
        data.userTel!.substring(3, 7) +
        '-' +
        data.userTel!.substring(7, 11);
    office = data.office!;
    officeAddress = data.officeAddr!;
    userName = data.userName!;
    nameCardUrl = data.ncardUrl!;
    imgNumList = [];
    imgList = [];
    imgDocList = ['', '', '', '', '', '', '', '', '', '', '', '', ''];
    imData123 = [];
    if (pdfDealType == 'L') {
      final MyDealDetailLandItem? ret2 =
          await IdApi.getMydaelDetail(userNo, dealNo, pdfDealType);
      title = ret2!.dealMaster!.title!;
      latitude = ret2.dealMaster!.latitude!;
      longitude = ret2.dealMaster!.longitude!;
      address = ret2.dealMaster!.address! + ' ' + ret2.dealMaster!.addressDtl!;
      ...
      for (var i = 0; i < ret2.fileList!.length; i++) {
        imgNumList.add(int.tryParse(ret2.fileList![i].fileOrder!) ?? 0);
      }
      for (var i = 0; i < imgNumList.length; i++) {
        imgList[(int.tryParse(ret2.fileList![i].fileOrder!) ?? 1) - 1] =
            ret2.fileList![i].s3FileUrl!;
        imgDocList[(int.tryParse(ret2.fileList![i].fileOrder!) ?? 1) - 1] =
            ret2.fileList![i].fileDoc!;
      }
    } else {
      final MyDealDetailBuildingItem? ret2 =
          await IdApi.getMydaelDetail(userNo, dealNo, pdfDealType);
      ...
      for (var i = 0; i < ret2.rentrollList!.length; i++) {
        rentDeposit = NumberFormat('#,##0.00').format(
            (double.tryParse(ret2.rentrollList![i].deposit!) ?? 0) / 10000);

        if (ret2.rentrollList![i].rent!.length >= 4) {
          rentRent = NumberFormat('#,##0.00').format(
              (double.tryParse(ret2.rentrollList![i].rent!) ?? 0) / 10000);
        } else {
    ...
    officeInfo = baseOffice(title, '$office $userName', phone, address);
    ...
    for (int i = 0; i < imData123.length; i += 11) {
      ...
      imData1 += double.tryParse(imData123[i][2].split('㎡')[0]) ?? 0;

      imData2 +=
          (double.tryParse(imData123[i][3].split(' ')[0].replaceAll(',', '')) ??
                  0) /
              10000;
      imData3 +=
          (double.tryParse(imData123[i][4].split(' ')[0].replaceAll(',', '')) ??
                  0) /
              10000;

      final page5 = await _tmBuildingPage5(format, font123b, font123r,
          imData123.sublist(i, l1 >= 11 ? i + 11 : i + l1),
          //TODO 합계
          total: l1 - 11 > 0
              ? null
              : [imData1.toString(), imData2.toString(), imData3.toString()]);
      lists.add(page5);
    }
    ...
    if (tmType == TmType.building)
      for (int i = 0; i < lvData123.length; i += 11) {
        ...
        final page6 = await _tmBuildingPage6(format, font123b, font123r,
            lvData123.sublist(i, l1 >= 11 ? i + 11 : i + l1));
        lists.add(page6);
      }

    final page7 = await _tmBuildingPage7(format, font123b, font123r,
        pictures.sublist(0, pictures.length > 4 ? 4 : pictures.length),
        imgs2:
            pictures.length > 4 ? pictures.sublist(4, pictures.length) : null);
    lists.add(page7);

    if (tmType == TmType.building) {
      if (imgDocList[11] != '') {
        final page8 = await _tmBuildingPage8(format, font123b, font123r,
            img11st, imgDocList[11].toString().split('.')[0]);
        lists.add(page8);
      }
    }
    final page9 =
        await _tmBuildingPage9(format, font123b, font123r, img1st, imgMap);
    lists.add(page9);
    ...
    for (var p1 in lists) {
      doc.addPage(pw.Page(
          pageTheme: p1,
          build: (context) =>
              pw.SizedBox(width: double.infinity, height: 425)));
    }
    return doc.save();
  }

  ///마지막장
  static Future<pw.PageTheme> _tmBuildingPage99(PdfPageFormat format,
      pw.Font font123b, pw.Font font123r, pw.MemoryImage img1) async {
    format = format.applyMargin(left: 0, top: 0, right: 0, bottom: 0);
    final backImg = pw.MemoryImage((await rootBundle.load(
            tmType == TmType.building
                ? 'assets/img/tm-building-end.png'
                : 'assets/img/tm-land-end.png'))
        .buffer
        .asUint8List());
                ...
                children: [
                  ...
                  pw.Positioned(
                      ...
                      child: pw.Container(
                          width: 600 / 2,
                          height: 360 / 2,
                          color: PdfColor.fromInt(0xD9D9D9))),
                  pw.Positioned(
                      right: 71 / 2,
                      bottom: 210 / 2,
                      child: pw.Container(
  ...
  ///첫장
  static Future<pw.PageTheme> _tmBuildingPage1(PdfPageFormat format,
      pw.Font font123b, pw.Font font123r, pw.MemoryImage img1) async {
    format = format.applyMargin(left: 0, top: 0, right: 0, bottom: 0);
    final backImg = pw.MemoryImage((await rootBundle.load(
            tmType == TmType.building
                ? 'assets/img/tm-building-head.png'
                : 'assets/img/tm-land-head.png'))
        .buffer
        .asUint8List());
    var clippedImage = pw.ClipOval(
        child: pw.Image(width: 400, height: 400, fit: pw.BoxFit.fill, img1));
            ...
            child: pw.Container(
              ...
              child: pw.Stack(
                children: [
                  pw.Container(child: pw.Image(backImg)),
                  pw.Positioned(
                      left: 838 / 2, top: 196 / 2, child: clippedImage),
                  pw.Positioned(
                    left: 113 / 2,
                    top: 980 / 2,
                    child: pw.Text(officeInfo!.office,
                        style: pw.TextStyle(
                            color: baseColor,
                            fontSize: 32 / 2,
                            font: font123b,
                  ...
                  pw.Positioned(
                    ...
                    child: pw.Container(
                      width: 640 / 2,
                      child: (officeInfo!.title.length > 7)
                          ? pw.Column(
                              ...
                              children: [
                                pw.Row(
                                  ...
                                  children: [
                                    pw.Text(officeInfo!.title.substring(0, 7),
                                        style: pw.TextStyle(
                                ...
                                pw.Row(
                                  ...
                                  children: [
                                    pw.Text(
                                        officeInfo!.title.substring(
                                            7, officeInfo!.title.length),
                                        style: pw.TextStyle(
                                            color: PdfColors.white,
                                            fontSize: 96 / 2,
                                            font: font123b,
  ...
  static Future<pw.PageTheme> _tmBuildingPage2(
      PdfPageFormat format, pw.Font font123b, pw.Font font123r) async {
    ...
    final backColor = tmType == TmType.building
        ? PdfColor.fromHex('D3CCBF')
        : PdfColor.fromHex('ADBDB9');
      ...
      buildBackground: (pw.Context context) {
        return pw.FullPage(
            ...
            child: pw.Container(
              ...
              child: pw.Stack(
                children: [
                  pw.Container(color: backColor
                      // child: pw.Image(backImg),
                      ),
                  pw.Positioned(
                    top: 0,
                    left: 60 / 2,
                  ...
                  pw.Positioned(
                    ...
                    child: pw.SizedBox(
                      ...
                      height: 731 / 2,
                      child: pw.Text(
                          '본 Teaser Memorandum(이하 “소개서”)은 "$address"에 위치한 토지 및 건물에 대하여 귀사의 매입 또는 투자를 위해 기본적인 정보를 제공할 목적으로 작성한 것이고, $office의 서면에 의한 사전동의 없이는 그 전부 또는 일부가 복사되거나 제3자에게 공개 또는 인용되지 않는다는 전제하에 이를 귀사에 제공합니다.\n\n본 소개서의 정보는 소유자가 제공한 자료와 $office이 확보한 자료, 기타 신뢰할 수 있는 시장정보 등을 근거로 작성하였습니다. 그러나 정보는 추후 진행 상황에 따라 변경될 수 있으며, 소개서에 포함된 어떠한 정보에 대하여도 당사 및 이해관계자는 정확성 및 완전성에 대하여 명시적으로 또는 묵시적으로 어떠한 의견 표명이나 보증을 하지 않으며, 귀사는 자기 책임하에 자체적 조사와 분석을 병행 해야 합니다.\n\n본 소개서는 법적으로 제출되거나 등록되지 아니하였으며, 독립적으로 검증된 절차를 거치지 아니하였습니다.\n\n본 소개서는 딜스테이션 서비스에서 $office이 입력한 정보를 통해 제작되었습니다. 딜스테이션은 본 소개서로 제공되는 정보 및 자료에 대하여 어떠한 책임도 지지 않으며, 본 소개서로부터 제3자 또는 잠재적 투자자의 손해나 피해에  대하여 어떠한 책임과 의무를 부담하지 않습니다.\n\n본 자산에 관심을 가져주셔서 진심으로 감사드리며, 추가적인 질의나 문의 사항이 있는 경우 담당자에게 연락주시기 바랍니다.',
                          style: pw.TextStyle(
                              color: PdfColor.fromInt(0xFF222222),
                              fontSize: 24 / 2,
                              font: font123b,
  ...
  static Future<pw.PageTheme> _tmBuildingPage3(PdfPageFormat format,
      pw.Font font123b, pw.Font font123r, pw.MemoryImage img1) async {
    format = format.applyMargin(
        left: 2.0 * PdfPageFormat.cm,
        top: 4.0 * PdfPageFormat.cm,
    ...
    if (pdfDealType == 'L') {
      ...
      for (var i = 0; i < ret2!.lotList!.length; i++) {
        k += double.tryParse(ret2.lotList![i].lotArea!) ?? 0;
      }
      lotArea = k.toString();
      ...
      reModel = '-';
      for (var i = 0; i < ret2.lotList!.length; i++) {
        j += double.tryParse(ret2.lotList![i].asking!) ?? 0;
      }
      ...
      if (ret2.dealMaster!.asking != null) {
        asking = ((double.tryParse(ret2.dealMaster!.asking!) ?? 0) / 100000000)
            .toString();
      } else {
        asking = '0';
      }
      deposit =
          ((double.tryParse(ret2.building!.deposit!) ?? 0) / 10000).toString();
      monthly =
          ((double.tryParse(ret2.building!.monthly!) ?? 0) / 10000).toString();
                ...
                children: [
                          ...
                          pw.Positioned(
                            ...
                            child: pw.Container(
                                ...
                                decoration: pw.BoxDecoration(
                                    color: PdfColor.fromInt(0xFFFFFF7C)),
                                child: pw.Image(img1)),
                          ),
                        ],
                      ),
                  ...
                  pw.Positioned(
                      ...
                      child: pw.Container(
                          ...
                          height: 1,
                          decoration: pw.BoxDecoration(
                              color: baseColor, shape: pw.BoxShape.rectangle))),
                  pw.Positioned(
                      left: 80 / 2,
                      top: 632 / 2,
                      child: pw.Container(
                  ...
                  pw.Positioned(
                      ...
                      top: 1110 / 2,
                      child: _pageTail(
                          tailMsg: officeInfo!.tailMsg(), font: font123r)),
                  pw.Positioned(
                      left: 80 / 2,
                      top: 230 / 2,
                      child: pw.Text('건축면적',
...
////
                  pw.Positioned(
                      ...
                      top: 650 / 2,
                      child: pw.Text(
                          '${NumberFormat('#,##0.00').format(double.tryParse(asking) ?? 0)} 억원',
                          style: pw.TextStyle(
                              color: PdfColor.fromInt(0xFFFB6915),
                              fontSize: 24 / 2,
                              font: font123r,
                              height: 0.10))),
                  pw.Positioned(
...
////
                  pw.Positioned(
                      ...
                      top: 710 / 2,
                      child: pw.Text(
                          ((double.tryParse(deposit) ?? 0) >= 10000)
                              ? ((double.tryParse(deposit) ?? 0) % 10000 == 0)
                                  ? '${IdStrUtil.toMoneyUnitKr(deposit).replaceAll(' ', '')}원'
                                  : '${IdStrUtil.toMoneyUnitKr(deposit).replaceAll(' ', '')}만원'
                              : '${IdStrUtil.toMoneyUnitKr(deposit).replaceAll(' ', '')}만원',
                          style: pw.TextStyle(
                              color: PdfColor.fromInt(0xFFFB6915),
                              fontSize: 24 / 2,
...
////
                  pw.Positioned(
                      ...
                      top: 770 / 2,
                      child: pw.Text(
                          ((double.tryParse(monthly) ?? 0) >= 10000)
                              ? ((double.tryParse(monthly) ?? 0) % 10000 == 0)
                                  ? '${IdStrUtil.toMoneyUnitKr(monthly).replaceAll(' ', '')}원'
                                  : '${IdStrUtil.toMoneyUnitKr(monthly).replaceAll(' ', '')}만원'
                              : '${IdStrUtil.toMoneyUnitKr(monthly).replaceAll(' ', '')}만원',
                          style: pw.TextStyle(
                              color: PdfColor.fromInt(0xFFFB6915),
                              fontSize: 24 / 2,
...
////
  ...
  static Future<pw.PageTheme> _tmBuildingPage4(PdfPageFormat format,
      pw.Font font123b, pw.Font font123r, pw.MemoryImage img1) async {
    final iconBuilding = pw.MemoryImage(
        (await rootBundle.load('assets/img/tm-icon-Buildings.png'))
            .buffer
            .asUint8List());
    final iconMapPinLine = pw.MemoryImage(
        (await rootBundle.load('assets/img/tm-icon-MapPinLine.png'))
            .buffer
            .asUint8List());
    final iconMapTrifold = pw.MemoryImage(
        (await rootBundle.load('assets/img/tm-icon-MapTrifold.png'))
            .buffer
            .asUint8List());
    final iconNote = pw.MemoryImage(
        (await rootBundle.load('assets/img/tm-icon-Note.png'))
            .buffer
            .asUint8List());
    final iconReceipt = pw.MemoryImage(
        (await rootBundle.load('assets/img/tm-icon-Receipt.png'))
            .buffer
            .asUint8List());
    ...
    if (pdfDealType == 'L') {
      ...
      double m = 0;
      for (var i = 0; i < ret2.lotList!.length; i++) {
        m += double.tryParse(ret2.lotList![i].lotArea!) ?? 0;
      }
      lotArea = m.toString();
      areaPurpose = ret2.lotList![0].areaPurpose!;

      for (var i = 0; i < ret2.lotList!.length; i++) {
        j += double.tryParse(ret2.lotList![i].officialLandPrice!) ?? 0;
      }
      officialLandPrice = j.toString();

      for (var i = 0; i < ret2.lotList!.length; i++) {
        k += double.tryParse(ret2.lotList![i].totalLandPrice!) ?? 0;
      }
      totalLandPrice = k.toString();

      for (var i = 0; i < ret2.lotList!.length; i++) {
        l += double.tryParse(ret2.lotList![i].asking!) ?? 0;
      }
      ...
      if (ret2.building!.totalFloorRatio != null) {
        totalFloorRatio = ret2.building!.totalFloorRatio!;
      } else {
        totalFloorRatio = ret2.building!.totalFloorRatio!;
      }
                  ...
                  pw.Positioned(
                    ...
                    child: pw.Container(
                      ...
                      decoration: pw.BoxDecoration(
                        ...
                        border: pw.Border(
                            top: pw.BorderSide(
                                width: 1, color: PdfColor.fromInt(0xFFEEEEEE))),
                      ),
                      child: pw.Row(
                        mainAxisSize: pw.MainAxisSize.min,
                        mainAxisAlignment: pw.MainAxisAlignment.center,
                  ...
                  pw.Positioned(
                    ...
                    child: pw.Container(
                      ...
                      padding: const pw.EdgeInsets.symmetric(
                          horizontal: 10 / 2, vertical: 12 / 2),
                      decoration: pw.BoxDecoration(
                        color: PdfColor.fromInt(0xFFF7F7F7),
                        border: pw.Border(
                          top: pw.BorderSide(
                              width: 2, color: PdfColor.fromInt(0xFF222222)),
                        ),
                      ),
                      child: pw.Row(
                        mainAxisSize: pw.MainAxisSize.min,
                        ...
                        children: [
                          ...
                          pw.Text(
                              IdStrUtil.toMoneyUnitKr(NumberFormat('#,##0.00')
                                      .format(
                                          (double.tryParse(totalLandPrice) ??
                                                  0) /
                                              10000)) +
                                  '만원',
                              style: pw.TextStyle(
                      ...
                      child: pw.Row(
                        ...
                        children: [
                          ...
                          pw.Text(
                              IdStrUtil.toMoneyUnitKr(NumberFormat('#,##0.00')
                                      .format(
                                          (double.tryParse(officialLandPrice) ??
                                                  0) /
                                              10000)) +
                                  '만원',
                              style: pw.TextStyle(
  ...
  ///임대정보, 11줄이 max
  static Future<pw.PageTheme> _tmBuildingPage5(PdfPageFormat format,
      pw.Font font123b, pw.Font font123r, List<List<String>> data123,
  ...
  ///층별정보, 11줄이 max
  static Future<pw.PageTheme> _tmBuildingPage6(PdfPageFormat format,
      pw.Font font123b, pw.Font font123r, List<List<String>> data123) async {
    format = format.applyMargin(left: 0, top: 0, right: 0, bottom: 0);
    final iconBuilding = pw.MemoryImage(
        (await rootBundle.load('assets/img/tm-icon-stack.png'))
            .buffer
            .asUint8List());
  ...
  ///사진
  static Future<pw.PageTheme> _tmBuildingPage7(PdfPageFormat format,
      pw.Font font123b, pw.Font font123r, List<pw.MemoryImage> imgs1,
      {List<pw.MemoryImage>? imgs2}) async {
    final iconImage = pw.MemoryImage(
        (await rootBundle.load('assets/img/tm-icon-Image.png'))
            .buffer
            .asUint8List());
  ...
  static Future<pw.PageTheme> _tmBuildingPage8(
    ...
    final iconImage = pw.MemoryImage(
        (await rootBundle.load('assets/img/tm-icon-appendix.png'))
            .buffer
            .asUint8List());
    return pw.PageTheme(
      pageFormat: format.applyMargin(left: 0, top: 0, right: 0, bottom: 0),
  ...
  static Future<pw.PageTheme> _tmBuildingPage9(
      ...
      pw.Font font123r,
      pw.MemoryImage img1,
      pw.MemoryImage imgMap) async {
    final iconNote = pw.MemoryImage(
        (await rootBundle.load('assets/img/tm-icon-Note.png'))
            .buffer
            .asUint8List());
    final iconMap = pw.MemoryImage(
        (await rootBundle.load('assets/img/tm-icon-MapPinLine.png'))
            .buffer
            .asUint8List());
    userNo = GV.pStrg.getXXX(Param_commonUserNo);
                      ...
                      child: pw.Row(
                        ...
                        children: [
                          pw.Container(
                            ...
                            height: double.infinity,
                            padding:
                                pw.EdgeInsets.symmetric(horizontal: 20 / 2),
                            decoration: pw.BoxDecoration(
                              color: PdfColor.fromInt(0xFFF7F7F7),
                              border: pw.Border(
                                  top: pw.BorderSide(
                          ...
                          pw.Container(
                            ...
                            height: double.infinity,
                            padding:
                                pw.EdgeInsets.symmetric(horizontal: 16 / 2),
                            decoration: pw.BoxDecoration(
                              border: pw.Border(
                                top: pw.BorderSide(
                                    width: 1,
                            ...
                            child: pw.Row(
                              ...
                              children: [
                                pw.SizedBox(
                                  width: 140 / 2,
                                  child: pw.Text(
                                      '${(double.tryParse(officialLandPrice) ?? 0) / 10000} 만원',
                                      textAlign: pw.TextAlign.center,
                                      style: pw.TextStyle(
                                          color: PdfColor.fromInt(0xFF222222),
                                          fontSize: 18 / 2,
        ...
        children: [
          ...
          pw.Positioned(
              ...
              child: pw.Container(
                  ...
                  height: 0.3,
                  decoration: pw.BoxDecoration(
                      color: TmPdf.baseColor, shape: pw.BoxShape.rectangle))),
        ],
      ),
    );
  }
          ...
          children: [
            pw.Positioned(
                ...
                child: pw.Container(
                    width: 20 / 2,
                    height: 190 / 2,
                    decoration: pw.BoxDecoration(color: headColor))),
            pw.Positioned(
                left: 60 / 2,
                top: 60 / 2,
                child: pw.Text(title,
