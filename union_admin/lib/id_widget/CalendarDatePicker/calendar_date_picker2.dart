...
import 'calendar_date_picker2_config.dart';
...
const double _yearPickerRowHeight = 32.0;
const double _yearPickerRowSpacing = 8.0;

const double _subHeaderHeight = 48.0;
...
class CalendarDatePicker2 extends StatefulWidget {
  CalendarDatePicker2({
    ...
    if (config.calendarType == CalendarDatePicker2Type.range && value.length > 1) {
      ...
      assert(isRangePickerValueValid,
...
class _CalendarDatePicker2State extends State<CalendarDatePicker2> {
  ...
  void initState() {
    ...
    final lastDate = widget.value.isNotEmpty && widget.value.length > 1 && widget.value[1] != null
        ? DateTime(widget.value[1]!.year, widget.value[1]!.month)
        : DateUtils.dateOnly(DateTime.now());
    _mode = config.calendarViewMode;
    _currentDisplayedMonthDate = config.calendarView == CalendarDatePicker2View.first
    ...
    setState(() {
      ...
      if (widget.config.calendarType == CalendarDatePicker2Type.single) {
        ...
        if (selectedDates.isEmpty) {
          ...
          if (widget.config.calendarView == CalendarDatePicker2View.none) {
          ...
          } else if (widget.config.calendarView == CalendarDatePicker2View.first) {
            selectedDates = [value, selectedDates[1]];
          } else {
            selectedDates = [selectedDates[0], value];
          }
        }
...
class _DatePickerModeToggleButton extends StatefulWidget {
  const _DatePickerModeToggleButton({required this.mode, required this.title, required this.onTitlePressed, required this.config});
...
class _DatePickerModeToggleButtonState extends State<_DatePickerModeToggleButton> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _controller = AnimationController(
        value: widget.mode == DatePickerMode.year ? 0.5 : 0, upperBound: 0.5, duration: const Duration(milliseconds: 200), vsync: this);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      padding: widget.config.centerAlignModePicker == true ? EdgeInsets.zero : const EdgeInsetsDirectional.only(start: 16, end: 4),
      height: (widget.config.controlsHeight ?? _subHeaderHeight),
      child: Row(
        children: <Widget>[
          if (widget.mode == DatePickerMode.day && widget.config.centerAlignModePicker == true)
              ...
              child: Semantics(
                  ...
                  child: SizedBox(
                      ...
                      child: InkWell(
                          ...
                          child: Padding(
                              padding: EdgeInsets.symmetric(horizontal: widget.config.centerAlignModePicker == true ? 0 : 8),
                              child: Row(
                                  mainAxisAlignment:
                                      widget.config.centerAlignModePicker == true ? MainAxisAlignment.center : MainAxisAlignment.start,
                                  children: <Widget>[
                                    Flexible(
                                        child: Text(widget.title,
                                            overflow: TextOverflow.ellipsis,
                                            style: widget.config.controlsTextStyle ?? textTheme.titleSmall?.copyWith(color: controlColor))),
                                    widget.config.disableModePicker == true
                                        ? const SizedBox()
                                        : RotationTransition(
                                            turns: _controller,
                                            child: widget.config.customModePickerIcon ??
                                                Icon(Icons.arrow_drop_down, color: widget.config.controlsTextStyle?.color ?? controlColor))
                                  ])))))),
...
class _DayPickerState extends State<_DayPicker> {
  ...
  List<Widget> _dayHeaders(TextStyle? headerStyle, MaterialLocalizations localizations) {
    ...
    for (int i = firstDayOfWeek; true; i = (i + 1) % 7) {
      ...
      result.add(ExcludeSemantics(child: Center(child: Text(weekday, style: widget.config.weekdayLabelTextStyle ?? headerStyle))));
  ...
  Widget build(BuildContext context) {
    ...
    final int dayOffset = getMonthFirstDayOffset(year, month, widget.config.firstDayOfWeek ?? localizations.firstDayOfWeekIndex);
    ...
    while (day < daysInMonth) {
      ...
      if (day < 1) {
        ...
        if (isDisabled) {
          customDayTextStyle = customDayTextStyle?.copyWith(color: disabledDayColor, fontWeight: FontWeight.normal);
        ...
        Widget dayWidget = widget.config.dayBuilder?.call(
                ...
                isToday: isToday) ??
            _buildDefaultDayWidgetContent(decoration, localizations, day, dayTextStyle);
            ...
            if (isDateInRange && !isStartDateSameToEndDate) {
              final rangePickerIncludedDayDecoration = BoxDecoration(
                  color: widget.config.selectedRangeHighlightColor ??
                      (widget.config.selectedDayHighlightColor ?? selectedDayBackground).withOpacity(0.08));

              if (DateUtils.isSameDay(startDate, dayToBuild)) {
                dayWidget = Stack(children: [
                  Row(children: [const Spacer(), Expanded(child: Container(decoration: rangePickerIncludedDayDecoration))]),
                  dayWidget
                ]);
              } else if (DateUtils.isSameDay(endDate, dayToBuild)) {
                dayWidget = Stack(children: [
                  Row(children: [Expanded(child: Container(decoration: rangePickerIncludedDayDecoration)), const Spacer()]),
                  dayWidget
                ]);
              } else {
                dayWidget = Stack(children: [Container(decoration: rangePickerIncludedDayDecoration), dayWidget]);
              }
            }
          }
        }

        dayWidget = Padding(padding: const EdgeInsets.symmetric(vertical: 1.8), child: dayWidget);
  ...
  Widget _buildDefaultDayWidgetContent(BoxDecoration? decoration, MaterialLocalizations localizations, int day, TextStyle dayTextStyle) {
    return Row(children: [
      const Spacer(),
      AspectRatio(
          aspectRatio: 1,
          child: Container(decoration: decoration, child: Center(child: Text(localizations.formatDecimal(day), style: dayTextStyle)))),
      const Spacer()
    ]);
  }
}
...
class _DayPickerGridDelegate extends SliverGridDelegate {
  ...
  SliverGridLayout getLayout(SliverConstraints constraints) {
    ...
    final double tileWidth = 34; //constraints.crossAxisExtent / columnCount;
    ...
    Widget yearItem = widget.config.yearBuilder?.call(
        ...
        Center(
            child: Container(
                ...
                child: Center(child: Semantics(selected: isSelected, button: true, child: Text(year.toString(), style: itemStyle)))));

    if (isDisabled) {
      yearItem = ExcludeSemantics(child: yearItem);
    } else {
      yearItem =
          InkWell(key: ValueKey<int>(year), onTap: () => widget.onChanged(DateTime(year, widget.initialMonth.month)), child: yearItem);
    }
